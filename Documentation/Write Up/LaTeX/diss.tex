...
In this section I assess the successes and failures of what was developed in this project. To follow a similar order to the implementation section, I first analyse the data in section \S\ref{eval-database}, then evaluate the classifier's results in section \S\ref{eval-results} and in section \S\ref{eval-reduction} I consider the benefits of the extension in which I implemented dimensionality reduction. I then move onto section \S\ref{eval-external} in which I compare the project's results to other similar projects, starting with the spam email dataset in \S\ref{eval-spam}. I conclude this chapter by assessing the extent to which the project met its goals in section \S\ref{eval-goals}.

\section{Internal Evaluation}
\subsection{Database Analysis} \label{eval-database}
...
\footnotetext{Although the standard error suggests it could be, the coefficient of determination cannot exceed 1.}
...
\subsection{Classifier Results} \label{eval-results}
...
The learning algorithm which determined the kernel and hyperparameters also determined the settings, which are shown in table \ref{table:settings}. 
...
\subsection{Evaluation of Dimensionality Reduction} \label{eval-reduction}

\section{External Evaluation} \label{eval-external}
